...
mv build/$name_all $HOME/.haqqd/cosmovisor/genesis/bin/
rm -rf build

# Create application symlinks
sudo ln -s $HOME/.haqqd/cosmovisor/genesis $HOME/.haqqd/cosmovisor/current -f
sudo ln -s $HOME/.haqqd/cosmovisor/current/bin/$name_all /usr/local/bin/$name_all -f
```
...
sed -i -e "s|^minimum-gas-prices *=.*|minimum-gas-prices = \"0.000000000001aISLM\"|" $HOME/.haqqd/config/app.toml
...
**RESET CHAIN DATA**
```bash
$name_all tendermint unsafe-reset-all --keep-addr-book --home $HOME/.haqqd --keep-addr-book
```
...
$name_all tx distribution withdraw-all-rewards --from wallet --chain-id $chain_id --gas-adjustment 1.4 --gas auto --gas-prices 1000000000000aISLM -y
# WITHDRAW COMMISSION AND REWARDS FROM YOUR VALIDATOR
$name_all tx distribution withdraw-rewards $(haqqd keys show wallet --bech val -a) --commission --from wallet --chain-id $chain_id --gas-adjustment 1.4 --gas auto --gas-prices 1000000000000aISLM -y
# Unjail Validator
$name_all tx slashing unjail --from wallet --chain-id $chain_id --gas-adjustment 1.4 --gas auto --gas-prices 1000000000000aISLM -y
```
